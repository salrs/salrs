...
//masterkeygen()
  for (ii = 0; ii < round; ++ii)
  {
        ...
        //printf("\norder in test:%d\n",ii);
	master_key_gen(mpk[0], msk[0]);
	derived_public_key_gen(mpk[0], ring[0]); 
...
//derivedkeygen() && ownercheck() && publiccheck()
  for (i = 0; i < round; ++i)
  {
		...
		ret = derived_public_key_owner_check(ring[0], msk[0], mpk[0]);
		if (ret == 0) {
		...
		ret = derived_public_key_public_check(ring[0]);
		if (ret == 0) {
...
**/
	...
	{
		master_key_gen(mpk[ii], msk[ii]);
		derived_public_key_gen(mpk[ii], ring[ii]); 
	}
...
/**        printf("\nOwner\n");
	for (ii = 0; ii < RSIZE[k]; ++ii)
	{
		...
		ret = derived_public_key_owner_check(ring[ii], msk[ii], mpk[ii]);
		if (ret == 0) {
			printf("\nDpk Owner Check Wrong\n\n");
			return -1;
		}
	...
	for (ii = 0; ii < RSIZE[k]; ++ii)
	{
		...
		ret = derived_public_key_public_check(ring[ii]);
		if (ret == 0) {
			printf("\nDpk Public Check Wrong\n\n");
			return -1;
		}
...
//           printf("Sign_test_%d\n",ii);
	   ...
	   ret = sign_salrs(m, MLEN, ring, RSIZE[k], ring[ii], mpk[ii], msk[ii], z, sig[ii]);
...
//         printf("Verify_test_%d\n", ii);
	   ...
	   ret = verify_salrs(m, MLEN, ring, RSIZE[k], z, sig[ii]);
...
/**    printf("Other_tests:\n\n");
	...
	derived_public_key_gen(mpk[0], ring[0]);
	sign_salrs(m, MLEN, ring, RSIZE, ring[0], mpk[0], msk[0], z, sm);
        printf("Test_link\n");
	ret = link_salrs(sm, RSIZE, z, sig[0], 10, z2);
	if (ret == 1) {
		printf("Double Spend\n");
		return -1;
	}
